...
class MultirotorConfig:
    ...
    def __init__(self, quadratic_thrust_curve_config={}):
        """
        ...
        self.thrust_curve = QuadraticThrustCurve(quadratic_thrust_curve_config)
...
class Multirotor(Vehicle):
    ...
    def update(self, dt: float):
        ...
        for i in range(self._thrusters._num_rotors):
            ...
            self.apply_force(
                [0.0, 0.0, forces_z[i]],
                body_part=self._thrusters._rotor_name_format.format(i),
            )
        ...
        self.apply_force(drag, body_part=self._thrusters._body_name)
    ...
    def handle_propeller_visual(self, rotor_number, force: float, articulation):
        ...
        joint = self.get_dc_interface().find_articulation_dof(
            articulation, self._thrusters._rotor_joint_name_format.format(rotor_number)
        )
    ...
    def force_and_torques_to_velocities(self, force: float, torque: np.ndarray):
        ...
        rb = self.get_dc_interface().get_rigid_body(
            self._stage_prefix + self._thrusters._body_name
        )

        # Get the rotors of the vehicle
        rotors = [
            self.get_dc_interface().get_rigid_body(
                self._stage_prefix + self._thrusters._rotor_name_format.format(i)
            )
            for i in range(self._thrusters._num_rotors)
        ]
